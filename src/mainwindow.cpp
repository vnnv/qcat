...
#define DISABLE_TEST_MENU

MainWindow::MainWindow()
{

	logger.debug("Constructing MainWindow", __FILE__, __LINE__);
    dm = NULL;
...
void MainWindow::readSettings()
{
	...
	logger.info(QString("Last used file is %1").arg(settings->lastFile), __FILE__, __LINE__);
...
{
	logger.info(QString("Loading file %1").arg(fileName), __FILE__, __LINE__);
	if (fileName.isEmpty())
	{
...
void MainWindow::search()
{
	...
	dlg.exec();
	int i = dlg.getReturnedId();
	if (i != -1){
		logger.debug("Result found.", __FILE__, __LINE__);
...
bool MainWindow::closeCatalog()
{
	...
	FREE_PTR(dm);
	logger.debug("Close catalog action.", __FILE__, __LINE__);
    return false;
}

void MainWindow::addMedia()
{
	logger.debug("Add Media action. Not implemented.", __FILE__, __LINE__);
    
}

void MainWindow::addGroup()
{
	logger.debug("Add Group action. Not Implemented.", __FILE__, __LINE__);
...
void MainWindow::testNewModel(){
	logger.debug("testNewModel invoked!", __FILE__, __LINE__);
	NewModelTest t;
	QCatItemModel *model = new QCatItemModel(dm);
	t.init(dm, model);
	t.exec();
}

void MainWindow::testNewModel1(){
	logger.debug("testNewModel1 invoked!", __FILE__, __LINE__);
	NewModelList t;
	QCatItemModel *model = new QCatItemModel(dm);
	t.init(dm, model);
	t.exec();
...
{
	logger.debug("Creating Actions", __FILE__, __LINE__);
	// File actions
...
//     connect(pasteAct, SIGNAL(triggered()), textEdit, SLOT(paste()));
	...
	testNewModelAct = new QAction(tr("Tests"), this);
	testNewModelAct->setStatusTip(tr("For tests"));
	testNewModelAct->setEnabled(true);
	connect(testNewModelAct, SIGNAL(triggered()), this, SLOT(testNewModel()));

	testNewModelAct1 = new QAction(tr("Test - List"), this);
	testNewModelAct1->setStatusTip(tr("For tests"));
	testNewModelAct1->setEnabled(true);
	connect(testNewModelAct1, SIGNAL(triggered()), this, SLOT(testNewModel1()));
}

void MainWindow::createMenus()
{
	logger.debug("Creating Menus", __FILE__, __LINE__);
...
#ifndef DISABLE_TEST_MENU
	menuBar()->addSeparator();
	testsMenu = menuBar()->addMenu(tr("&Tests"));
	testsMenu->addAction(testNewModelAct);
	testsMenu->addAction(testNewModelAct1);
#endif
...
{
	logger.debug("Creating Toolbars", __FILE__, __LINE__);
