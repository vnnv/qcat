...
class QCatDataModuleClass{
	public:
		...
		QList<QCatItem*> readCatalogs(QCatItem *rootItem, bool recursive);
		QList<QCatItem*> readItemsFromCatalog(int catId, int parentId, bool readFiles, bool recursive);
		QList<QCatItem*> readItemsFromCatalog(QCatItem *parent, bool readFiles, bool recursive);
	...
	private:
		int errorCode;
		QString errMsg;
		void fillDataFromQueryForItem(QSqlQuery *query, QCatItem *item, int catId);
		void fillDataFromQueryForCatalog(QSqlQuery *query, QCatItem *item);
