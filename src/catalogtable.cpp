...
void CatalogTable::vItemActivated(QTreeWidgetItem * activeItem, int column)
{
    ...
    if (item)
    {
        ...
        {
			logger.debug("The selected element is 'FILE'", __FILE__, __LINE__);
        }else{
			logger.debug("Item activated", __FILE__, __LINE__);
            emit goDown(item->id);   
            this->setCurrentItem(this->topLevelItem(0));
        }
...
#if defined(__i386__)
	char ptrCh[sizeof(int)];
	int listAddr = reinterpret_cast <int> (&selectedItemsList);
	memcpy(&ptrCh, &listAddr, sizeof(int));
	QByteArray ptr(ptrCh);
#elif defined(__x86_64__)
	char ptrCh[sizeof(long)];
	int listAddr = reinterpret_cast <long> (&selectedItemsList);
	memcpy(&ptrCh, &listAddr, sizeof(long));
	QByteArray ptr(ptrCh);
#endif
...
void CatalogTable::contextMenuEvent(QContextMenuEvent *event)
{
	logger.debug("Right button clicked.", __FILE__, __LINE__);
