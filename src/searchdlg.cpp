...
SearchDlg::~SearchDlg()
{
	logger.debug("Search dialog is destroyed. ~SearchDlg()");
}
...
//	if (q->isValid()){
//		qDebug() << "*** The query seems valid()";
//	}else{
//		qDebug() << "*** The query is not Valid()";
//	}
//	qDebug() << "Got " << q->size() << " records.";
	while (q->next())
    {
//		qDebug() << "Id = " << q->value(0).toString();
//		qDebug() << "Parent = " << q->value(1).toString();
//		qDebug() << "ItemType = " << q->value(2).toString();
//		qDebug() << "Name = " << q->value(3).toString();
//		qDebug() << "Size = " << q->value(4).toString();
//		qDebug() << "DateTime = " << q->value(5).toString();
//		qDebug() << "IconId = " << q->value(6).toString();
//		qDebug() << "Attribs = " << q->value(7).toString();
//		qDebug() << "doskNo = " << q->value(8).toString();
//		qDebug() << "------------------------------------";
		qlonglong _size = 0;
        int diskNo = q->value(8).toInt();
        if (!diskNames.contains(diskNo))
...
//            item->setText(0, q->value(1).toString());
		...
		item->fullType = 0; // TODO: Fill the actual type of the disk
        item->iconId = q->value(6).toInt();
        item->itemType = q->value(2).toInt();
        item->id = q->value(0).toInt();
        item->parentId = q->value(1).toInt();
