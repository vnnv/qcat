...
void CatalogTree::deleteDisk(bool dontAsk)
{
	...
	{
		logger.debug("Delete Disk", __FILE__, __LINE__);
		dbNode data;
...
void CatalogTree::deleteGroup(bool dontAsk)
{
	...
	{
		logger.debug("Delete Group", __FILE__, __LINE__);
...
{
	logger.debug("Entering dragEnterEvent()", __FILE__, __LINE__);
...
{
	logger.debug("Entering dragLeaveEvent()", __FILE__, __LINE__);
	stopAnimate();
}
...
void CatalogTree::dropEvent(QDropEvent * event)
{
    if (event->source() == this)
    {
		...
		if (moveCatTreeItem(draggedItem, _currentItem, true))
			logger.debug("Drag'n'Drop - Moved!", __FILE__, __LINE__);
		else
			logger.debug("Drag'n'Drop - Failed move!", __FILE__, __LINE__);
    }else{
		logger.debug("Using remote drag'n'Drop", __FILE__, __LINE__);
		...
		for (int i = 0; i < strList.count(); i++)
		{
			...
			logger.debug(QString("moving item %1").arg(idForMove), __FILE__, __LINE__);
			CatTreeItem *it = NULL;
			int childNo = -1;
			for (int j = 0; j < currentItem->childCount(); j++)
			{
				...
				if (it->id == idForMove)
				{
					logger.debug("moving item " + it->text(0), __FILE__, __LINE__);
					childNo = j;
					break;
				}
			}
			if (childNo != -1)
			{
				if (moveCatTreeItem(it, dropPlace, true))
					...
					logger.debug(QString("Drag'n'Drop - Failed to move item %1").arg(it->text(0)), __FILE__, __LINE__);
...
void CatalogTree::startDrag()
{
	...
	if ((draggedItem->itemType == GROUP_ELEMENT) || (draggedItem->itemType == DISK_ELEMENT))
	{
		...
		if (drag->start(Qt::MoveAction) == Qt::MoveAction)
			logger.debug("Drage Moved", __FILE__, __LINE__);
        autoOpenTimer->stop();
        stopAnimate();
...
void CatalogTree::animate()
{
    if (dropTargetItem)
    {
		QString icon = ":icons/openfolder/openfolder-act" + QString::number(currAnimFrame) + ".png";
		dropTargetItem->setIcon(0, QIcon(icon));
		//printQS(icon);
        currAnimFrame++;
        if (currAnimFrame > 8)
            currAnimFrame = 1;
